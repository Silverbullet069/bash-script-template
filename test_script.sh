...
@test "success() function logs success message" {
    run "${BATS_TEST_DIRNAME}/${SCRIPT_NAME}" "success" "Test success message"
    [[ "$status" -eq 0 ]]
    [[ "$output" == *"[SUC]"* ]]
...
@test "info() function logs info message" {
    run "${BATS_TEST_DIRNAME}/${SCRIPT_NAME}" "info" "Test info message"
    [[ "$status" -eq 0 ]]
    [[ "$output" == *"[INF]"* ]]
...
@test "warn() function logs warning message" {
    run "${BATS_TEST_DIRNAME}/${SCRIPT_NAME}" "warn" "Test warning message"
    [[ "$status" -eq 0 ]]
    [[ "$output" == *"[WRN]"* ]]
...
@test "error() function logs error message and exits" {
    run "${BATS_TEST_DIRNAME}/${SCRIPT_NAME}" "error" "Test error message"
    [[ "$status" -eq 0 ]]
    [[ "$output" == *"[ERR]"* ]]
